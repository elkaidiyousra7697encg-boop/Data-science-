...
L'objectif principal est de construire un modèle de régression linéaire multiple pour prédire l'espérance de vie (Life Expectancy) à partir de variables socio-économiques et sanitaires, et d'évaluer l'impact crucial de la normalisation des données sur la performance du modèle .
...
Chargement des données (adapter le nom exact du fichier depuis Kaggle)
link = "global_health_data.csv" # Nom à adapter selon le fichier téléchargé
df = pd.read_csv(link)
...
print(df.describe())
```
* Nombre d'échantillons ($N$) : ~2938 observations (typique pour ce type de dataset santé globale).
* Nombre de variables ($d$) : 20+ colonnes (features socio-économiques + target).
* Variables d'entrée ($X$) : GDP, BMI, alcool consommation, dépenses santé (%GDP), urbanisation, éducation, etc.
* Variable de sortie ($Y$) : Espérance de vie (Life Expectancy) en années .

### 2.2 Distribution de la Variable Cible

L'espérance de vie montre une distribution centrée autour de 70-75 ans pour la plupart des pays, avec des valeurs extrêmes pour les pays en développement (<50 ans) et développés (>80 ans).

| Espérance de Vie | Fréquence Relative |
|------------------|-------------------|
|50-60 ans        |15%                |
|60-70 ans        |35%                |
|70-80 ans        |40%                |
...
Les corrélations révèlent que l'espérance de vie est fortement corrélée positivement au GDP et à l'éducation, mais négativement au BMI et à la mortalité infantile.
```python
...
plt.show()
```
### 2.4 Préparation des Données pour la Régression
...
Sur données brutes, le GDP (échelle 10^9) domine les distances et biais le modèle.
```python
